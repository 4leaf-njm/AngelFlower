...
public class OrderController {
	...
	public String cart(Model model, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "order/cart";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
			...
			if(cartList != null) {
				...
				for(CartVO cart : cartList) {
					if(cart.getMemId() == null || cart.getMemId().equals("")) 
						totalPrice += cart.getProdPrice1() * cart.getQuantity();
					else {
						totalPrice += cart.getProdPrice1() * cart.getQuantity();
	...
	public String cart(CartVO cart, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
			url = "redirect:/home.do";
	...
	public String cart(WishVO wish, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String order(Model model, HttpSession session, RedirectAttributes rttr) throws SQLException{
	...
	public String removeCart(@RequestParam("delList") List<Integer> delList, HttpSession session, RedirectAttributes rttr) throws SQLException{
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String removeCartAll(String id, HttpSession session, RedirectAttributes rttr) throws SQLException{
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String addCartFromWish(@RequestParam("wishNo") int wishNo, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String removeWish(@RequestParam("wishNo") int wishNo, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String removeOrderCart(@RequestParam("cartNo") int cartNo, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "redirect:cart.do";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
	...
	public String directOrder(CartVO cart, @RequestParam("menu") int menu, HttpSession session, RedirectAttributes rttr) throws SQLException {
		...
		if(loginType != null && loginType.equals("admin")) {
			...
			if(member == null) {
				...
				if (cartList == null) {
					...
					url = "redirect:/product/detail.do?menu=" + menu + "&no=" + cart.getProdNo() + "&direct=y";
				}
	...
	public String complete(@RequestParam("no") int orderNo, Model model, HttpSession session, RedirectAttributes rttr) throws SQLException {
		String url = "order/order_complete";
		String loginType = (String) session.getAttribute("loginType");
		if(loginType != null && loginType.equals("admin")) {
			rttr.addFlashAttribute("msg", "기업회원은 이용하실 수 없습니다.");
			url = "redirect:/home.do";
			...
			for(OrderVO o : orderList) {
				if(o.getMemId() == null || o.getMemId().equals("")) 
	...
	public String complete(OrderVO order, String hope_date, HttpSession session, RedirectAttributes rttr) throws SQLException {
		...
		if(loginType != null && loginType.equals("admin")) {
			...
			url = "redirect:complete.do?no=" + orderNo;
		}
		return url;
	}
}